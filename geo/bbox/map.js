(function() {
    ...
    function updateInfo(bounds) {
        ...
        if (bounds) {
            ...
            // No redirect given -> Just display
            if(url.searchParams.get("redirect") == null) {
                alert(bounds);

            // Redirect
            } else {
                window.location.href = url.searchParams.get("redirect") + '?top=' + bounds.top + '&left=' + bounds.left + '&bottom=' + bounds.bottom + '&right='+bounds.right + '&' + url.searchParams.get("key") + '=' + url.searchParams.get("value");
            }
